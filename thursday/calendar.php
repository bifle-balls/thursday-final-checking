...
$query = "SELECT appointment_date, appointment_start_time, appointment_end_time, first_name, last_name 
...
function showAppointments(date) {
    ...
    if (appointments[date]) {
        ...
        content += '<table>';
        content += '<thead><tr><th>Patient Name</th><th>Start Time</th><th>End Time</th><th>Status</th></tr></thead>';
        content += '<tbody>';
        ...
        appointments[date].forEach(appointment => {
            ...
            <tr>
                ...
                <td>
                    <select class="status-dropdown" onchange="updateStatus(event, '${date}', '${appointment.name}')">
...
function updateStatus(event, date, name) {
    const newStatus = event.target.value;
    // Here you would need to update the status in the database via an AJAX request or form submission
    console.log(`Updating status for ${name} on ${date} to ${newStatus}`);
}
</script>
